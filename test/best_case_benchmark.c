...
// best cases --> insert at head every time
// search in increasing order
// remove from the head everytime
...
int main(void) {
    ...
    uint64_t test_sizes[] = {100, 500, 1000, 5000, 10000, 50000, 100000,500000,1000000};
    size_t n_sizes = sizeof(test_sizes) / sizeof(test_sizes[0]);

    FILE *csv = fopen("bestcase_benchmark_results.csv", "w");
    if (!csv) {
        perror("fopen");
        return 1;
    }
    ...
    fclose(csv);
    printf("\n✅ Benchmark results saved to bestcase_benchmark_results.csv\n");
    return 0;
}